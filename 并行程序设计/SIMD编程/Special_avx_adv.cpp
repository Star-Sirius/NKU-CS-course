...
const int n[10] = {96,192,288,384,512,608,704,800,896,992};
const int m[10] = {96,192,288,384,512,608,704,800,896,992};
...
int main()
{
	for(k=0;k<10;k++)
	{
	int *R = (int*) aligned_malloc(32*n[k]*n[k],32);
	int *E = (int*) aligned_malloc(32*n[k]*n[k],32);
	for(int i=0;i<n[k];i++)
		for(int j=0;j<n[k];j++)
		{
			R[i*n[k]+j] = 0;
	...
	for(int i=0;i<m[k];i++)
	{
		...
		{
			IsNullHelpE = _mm256_load_si256((__m256i_u*)&E[i*n[k]+j]);
		...
		while(IsNull)
		{
			...
			for(int j=0;j+8<n[k];j+=8)
			{
				IsNull2HelpR = _mm256_load_si256((__m256i_u*)&R[lp(&E[i*n[k]])*n[k]+j]);
			...
			if(IsNull2)
			{
				...
				for(int j=0;j+8<n[k];j+=8)
				{
					vtE = _mm256_load_si256((__m256i_u*)&E[i*n[k]+j]);
					vtR = _mm256_load_si256((__m256i_u*)&R[FirstNum*n[k]+j]);
					vtE = _mm256_xor_si256(vtE,vtR);
					_mm256_store_si256((__m256i_u*)&E[i*n[k]+j],vtE);
				}
			}
			else
			{
				...
				{
					vtE = _mm256_load_si256((__m256i_u*)&E[i*n[k]+j]);
					_mm256_store_si256((__m256i_u*)&R[FirstNum*n[k]+j],vtE);
					_mm256_store_si256((__m256i_u*)&E[i*n[k]+j],vtE2);
				}
				break;
			}
